...
with st.sidebar:
    ...
    st.divider()
    st.caption("Production App v2.8 | 2026")
    ...
    with col1:
        ...
        if not df_p.empty:
            for i, row in df_p.iterrows():
                ...
                c_text.write(f"🔹 **{row['concept']}** ({row.get('platform', 'Long')})")
...
# --- HALAMAN 2: TAMBAH RENCANA ---
    ...
    with st.container(border=True):
        ...
        if st.button("🚀 Simpan ke To-Do List", use_container_width=True):
            if new_concept:
                supabase.table("content_plans").insert({"concept": new_concept, "category": category, "platform": platform, "target_week": target_week, "status": "Plan"}).execute()
...
# --- HALAMAN 3: PORTAL EDITOR ---
    ...
    if not df_u.empty:
        ...
        video_file = st.file_uploader("Pilih file video (MP4/MOV):", type=["mp4", "mov"])
        ...
        st.info("Tidak ada rencana konten di To-Do List. Tambahkan rencana dulu!")
    ...
    if not df_r.empty:
        for i, row in df_r.iterrows():
            with st.container(border=True):
                ...
                # --- CATATAN REVISI ---
                catatan = st.text_area("Catatan untuk Editor:", key=f"txt_rev_{i}", placeholder="Tulis instruksi revisi di sini...")
                ...
                if c2.button("✍️ REVISI", key=f"r_{i}", use_container_width=True):
                    st.warning(f"Revisi tercatat: {catatan}")
        ...
        st.info("Belum ada video baru untuk direview.")

...
with st.sidebar:
    ...
    st.divider()
    st.caption("Production App v3.0 | Final Edition")

# Ambil data terbaru
res = supabase.table("content_plans").select("*").execute()
df = pd.DataFrame(res.data) if res.data else pd.DataFrame(columns=["concept", "status", "category", "platform", "target_week", "notes"])
        ...
        if not df_p.empty:
            for i, row in df_p.iterrows():
                with st.container():
                    ...
                    # --- FITUR BARU: INFO REVISI ---
                    if row.get('notes'):
                        c_text.markdown(f"""<div class='revisi-box'><b>🔥 REVISI:</b> {row['notes']}</div>""", unsafe_allow_html=True)
                    
                    c_text.caption(f"📌 {row.get('platform', 'Long')} | Week {row.get('target_week', '-')} | {row.get('category', 'General')}")
        ...
        if st.button("🚀 Simpan ke To-Do List", use_container_width=True):
            if new_concept:
                supabase.table("content_plans").insert({
                    "concept": new_concept, "category": category, 
                    "platform": platform, "target_week": int(target_week), 
                    "status": "Plan", "notes": "" # Notes dikosongkan saat baru buat
                }).execute()
...
# --- HALAMAN 3: EDITOR ---
    ...
    if not df_u.empty:
        ...
        if st.button("🚀 Kirim ke Bos"):
            if video_file:
                # Saat upload ulang, kosongkan catatan revisi yang lama
                ...
                st.success("Terkirim! Catatan revisi telah dibersihkan.")
                st.rerun()
